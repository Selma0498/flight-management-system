...
class LuggagemanagementKafkaResourceIT {
    ...
    void setup() {
        ...
        LuggagemanagementKafkaResource kafkaResource = new LuggagemanagementKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/luggagemanagement-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/luggagemanagement-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
