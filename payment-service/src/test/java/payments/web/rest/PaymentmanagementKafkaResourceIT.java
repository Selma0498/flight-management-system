...
class PaymentmanagementKafkaResourceIT {
    ...
    void setup() {
        ...
        PaymentmanagementKafkaResource kafkaResource = new PaymentmanagementKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/paymentmanagement-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/paymentmanagement-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
